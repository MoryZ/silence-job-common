...
public class LarkAlarm extends AbstractAlarm<AlarmContext> {
    ...
    public NotifyType getAlarmType() {
        return NotifyType.LARK;
    }

    @Override
    public boolean syncSendMessage(AlarmContext context) {
        try {
            LarkAttribute larkAttribute = JSON.parseObject(context.getNotifyAttribute(), LarkAttribute.class);
            ...
            HttpRequest request = post.body(JSON.toJSONString(builder), ContentType.JSON.toString());
            HttpResponse execute = request.execute();
            if (execute.isOk()) {
                return true;
            }
            SilenceJobLog.LOCAL.error("发送lark消息失败:{}", execute.body());
            return false;
        } catch (Exception e) {
